...
Now, you may be eager for an in-depth explanation of a real hardware memory model for a specific CPU. But I'm not going to give an explanation, because I believe that promotes misunderstanding. Some folks are under the impression that because they know a hardware memory model, their knowledge is sufficient to create thread safe software. This is a fallacy. Firstly, hardware is an extremely involved topic - it is unlikely you truly have or could achieve a complete understanding. Secondly, and more importantly, the hardware level is just the wrong system for us to interface with, as high level language programmers.[^4] Hardware provides valuable context, but we don't write code for hardware, we write it for the compiler. And so, the real gatekeeper is the memory model of your programming language, which we will discuss next.

[^4]: I have seem people claim, for example, that cache coherency is the reason for needing `std::atomic`. Yet, on the ubiquitous x86 architecture, the hardware topics we covered really don't play that large of a role in thread safety. The x86 memory model is one of the most CPU-programmer-friendly out there, with much solved invisibly by the hardware. Cache coherency is basically not a concern from the programmer's perspective, and most memory instruction reorderings are disallowed. Only unaligned memory access incurs some thread safety hazards. We do pay a continual cost in performance, efficiency, and chip complexity in exchange for such a friendly model. Newer chips, such as ARM, have shifted the tradeoff closer to efficiency.
